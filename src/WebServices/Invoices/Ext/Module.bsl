...
Функция PrintDocuments(DocumentsRef, Stamp, Options)
	...
	Для Каждого ЭлементСписка Из ТипыДокументов Цикл
		...
		Если ЭлементСписка = "Invoice" Тогда
			ИмяМакета = "СчетЗаказ" + ?(Stamp, "СПечатью", "");
		...
		ИначеЕсли ЭлементСписка = "InvoiceFacture" Тогда
			...
			ИмяМакета = УчетНДСВызовСервера.ПолучитьНастройкиПечатиСчетовФактур(МассивОбъектов).СписокМакетов + ?(Stamp, "СПодписью", "");
		КонецЕсли;
		...
		Если ЭлементСписка = "Act" Тогда
			...
			Если Stamp Тогда	
				ДвоичныеДанныеПечати              = Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(ДокументСсылка.Организация, "ФайлПечать");
				ДвоичныеДанныеПодписьРуководителя = Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(ДокументСсылка.Организация, "ФайлПодписьРуководителя");
				
				ВывестиКартинкуВТабличныйДокумент(Таб, ИндексСтроки, 9, 8, 7, 43, 43, ДвоичныеДанныеПечати);
				ВывестиКартинкуВТабличныйДокумент(Таб, ИндексСтроки + 4, 5, 4, 8, 44, 23, ДвоичныеДанныеПодписьРуководителя);
			КонецЕсли;
...
Функция PrintReconciliationAct(Data, Stamp)
	...
	ИмяМакета = "АктСверки" + ?(Stamp, "СПечастьюИПодписью", "");
	
	МенеджерПечати = Документы.АктСверкиВзаиморасчетов;
	Таб = ПолучитьТабличныйДокумент(АктыСверки, ИмяМакета, МенеджерПечати);
	Таб.АвтоМасштаб = Истина;
...
Функция GetContract(ContractID)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ДанныеДоговора) Тогда
			...
			ЗаполнитьЗначенияСвойств(Contract, ДанныеДоговора);
		КонецЕсли;
	Исключение
...
Функция НайтиАктСверки(ПараметрыПоиска)
	...
	|	И (НЕ &ДоговорУказан ИЛИ АктСверкиВзаиморасчетов.ДоговорКонтрагента = &ДоговорКонтрагента)
